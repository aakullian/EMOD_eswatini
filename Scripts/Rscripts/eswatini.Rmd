...
network.data.m <- network.data %>%    
  ...
  mutate(MonthsToTransmission = round((YEAR - acq.year)*12, 0)) %>%
...
network.data.m %>% group_by(scenario, sim.id, SRC_GENDER, acq.year) %>% 
  complete(Month = seq(min(MonthsToTransmission), max(MonthsToTransmission), "day"), fill = list(Value = 0)) 

test <- 
  network.data.m %>%  filter(!is.na(acq.year)) %>% mutate(acq.year = floor(acq.year)) %>% complete(MonthsToTransmission, nesting(scenario, sim.id, SRC_GENDER, acq.year), fill = list(count_number = 0))
...
#%>% group_by(scenario, SRC_GENDER, acq.year, TimeToTransmission) %>% summarize(med.transperperson=median(transperperson), med.count=median(count), med.transmissions=median(transmissions)) 

timeframe = 12 #time horizon
ttable.selection <- complete(subset(ttable, MonthsToTransmission<timeframe & acq.year%in%seq(1985,2050,5) & scenario=="baseline"), MonthsToTransmission = 1:timeframe, fill = list(transperperson = 0)) #fill in missing month and add zeros
library(MESS)

ttable.selection.med <- ttable.selection %>% group_by(scenario, SRC_GENDER, acq.year, MonthsToTransmission) %>% summarize(med.transperperson=median(transperperson))

ttable.selection.med.auc <- ttable.selection.med %>%
    group_by(scenario, SRC_GENDER, acq.year) %>%
    summarize(AUC = auc(MonthsToTransmission,med.transperperson, type = "spline"))

labs <- c("1" = "Women", "0" = "Men")
ggplot(ttable.selection) + 
  geom_point(aes(y=transperperson, x=MonthsToTransmission, color=factor(acq.year), group=interaction(sim.id, acq.year)), size=1,  alpha=0.05) +
  geom_line(aes(y=transperperson, x=MonthsToTransmission, color=factor(acq.year), group=interaction(sim.id, acq.year)), size=1, alpha=0.05) +
  geom_smooth(aes(x=MonthsToTransmission, y=transperperson),method="loess", span=0.3, se = F, size=1, linetype=1, color="black") +
  facet_grid(acq.year~SRC_GENDER, labeller=labeller(SRC_GENDER=labs))+
  scale_x_continuous(breaks=seq(0,timeframe-1,1)) + scale_y_continuous(expand = c(0,0), breaks=seq(0,0.30,0.05)) + 
