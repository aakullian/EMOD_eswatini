...
ggplot(subset(ttable.yearscumsum, YearsToTransmission%in%seq(1,30,1))) + 
  ...
  #scale_color_manual(values = c("blue", "red"), labels = c("Male", "Female"))+
  #scale_fill_manual(values = c("blue", "red"), labels = c("Male", "Female"))+
  ggtitle("Number of transmissions per infected individual by year from HIV acquisition") +
  theme(strip.background = element_rect(colour="black", fill="white")) +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        legend.title=element_blank(),
        panel.grid.minor = element_blank(),
...
ggplot(ttable.yearscumsum) + 
  ...
  stat_contour(aes(y=YearsToTransmission, x=acq.year, z=med.reff), color="black", size=1, linetype=1, binwidth=1) +
