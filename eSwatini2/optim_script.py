...
def load_campaign_templates(template_dir):
    ...
    campaign_file_regex = re.compile("^campaign_.+\.json$")  # Original
    campaign_templates = {}
...
TEST_N = 99  # TEST_N is macro variable used to create directory name
...
# before resurrection redux (4/7/2021) was InputsFor2020
...
def resolve_scenario_template_set(template_set_name, campaign_template_name):
    ...
    if campaign_template_name is None:  # grab the one and only campaign file, error if > 1 of them
        ...
        if campaign_template is not None:
            ...
            raise Exception('Unknown campaign specified: %s for template set: %s' % (campaign_template_name, template_set_name))
